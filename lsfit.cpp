...
double glsfstat(int p, const std::vector<double> &y, const std::vector<double> &x, const std::vector<double> &v)
{
    int info = 0;
    ...
    if (p == 1) {
        ...
        auto k = static_cast<int>(q) - p;
        for (int i = 0; i < p; ++i) {
            auto s1 = i * p;
            auto s2 = (k + i) * static_cast<int>(q);
            for (int j = 0; j < p; ++j)
